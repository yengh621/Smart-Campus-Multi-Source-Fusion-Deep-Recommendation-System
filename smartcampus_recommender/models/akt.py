"""AKT风格时序学习编码器：因果自注意力、时间衰减特征和答题状态联合建模。"""
import torch
...
class AKTEncoder(nn.Module):
    def __init__(self, num_questions: int, num_concepts: int, dim: int, heads: int,
        ...
        self.layers = nn.ModuleList([
            TimeDecayAttentionLayer(dim, heads, dropout, time_decay_kernels) for _ in range(layers)])
    ...
    def forward(self, questions, concepts, correct, intervals, night, wrong_streak,
                ...
                kg_concept_embedding=None):
        numeric = torch.stack([night, wrong_streak], dim=-1)
        x = (self.question_emb(questions) + self.concept_emb(concepts) +
             self.correct_emb(correct.long().clamp(0, 1)) + self.numeric(numeric) +
             # Time2Vec 使用 log1p 小时以控制数值尺度；衰减核仍读取真实小时差。
             self.time2vec(torch.log1p(intervals)) + self.relative_position(relative_position.clamp_max(
                 self.relative_position.num_embeddings - 1)))
        ...
        if empty.any():
            safe_mask[empty, 0] = True  # 防止全padding注意力产生NaN，最终池化仍使用原mask。
        for layer in self.layers:
            hidden = layer(hidden, intervals, ~safe_mask)
        hidden = self.norm(hidden)
        kt_logits = self.kt_head(hidden).squeeze(-1)
        weights = mask.float()
        z_stu = (hidden * weights.unsqueeze(-1)).sum(1) / weights.sum(1, keepdim=True).clamp_min(1.0)
